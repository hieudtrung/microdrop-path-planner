...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QTextEdit, \
     ...
     QSplitter, QSizePolicy, QFileDialog, QDialog, QLabel
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        configure_action = QAction('Open Configuration', self)
        configure_action.triggered.connect(self.showConfigureDialog)  # Connect the action to the method
    ...
    def showConfigureDialog(self):
        ...
        result = configure_dialog.exec_()

        if result == QDialog.Accepted:
            # Handle the configuration input (for example, print it)
            config_text = configure_dialog.text_input.text()
            print(f'Configuration entered: {config_text}')
